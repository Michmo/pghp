...
{
    //parcours des sommets pour mettre toutes les normales à 0.
    for(VertexArray::iterator vx=mVertices.begin(); vx!=mVertices.end(); ++vx)
        vx->normal = Vector3f::Zero();
    //parcours des faces pour remplir les sommets adjacents.
    for(FaceIndexArray::iterator f=mFaces.begin(); f!=mFaces.end(); ++f){

        Eigen::Vector3f n = (mVertices[(*f)(0)].position - mVertices[(*f)(1)].position).cross(mVertices[(*f)(1)].position - mVertices[(*f)(2)].position);
        for (int i=0; i<3; ++i)
            mVertices[(*f)(i)].normal += n;
    }
    //normalisation des normales de tous les sommets.
    for(VertexArray::iterator vx=mVertices.begin(); vx!=mVertices.end(); ++vx)
        vx->normal.normalize();
