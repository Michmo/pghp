...
using namespace std;
  ...
  **/
void generateField(int w, int h, char *filename, PgmReader pr){
    ofstream file;
...
int main(){
    ...
    double elapsed;
    start = clock();
    cout << pr->getPixValues().size() << " " << pr->getWidth() << " " << pr->getHeight() << endl;
    generateField(pr->getWidth(), pr->getHeight(),"../data/test.off", *pr);
    end = clock();
